...
% Supports extended sources and detectors. Modifications to contents of the
% SrcData and Det fields
...
for s =1:NofSources
    if SrcData(s).ID=='S'
        ...
        % mesh.hx or mesh.hy
        xs = SrcData(s).Loc(1);ys = SrcData(s).Loc(2);
        if ~isfield(SrcData(s),'Radius')
            SrcData(s).Radius = 0.01*min(mesh.hx,mesh.hy); % set default radius to smaller than mesh size . This simulates a point source
        elseif isempty(SrcData(s).Radius)
             SrcData(s).Radius = 0.01*min(mesh.hx,mesh.hy);
        end
        xflag = (xs==xmin)||(xs==xmax); % '1' - source lies on x = constant edge ; '0' - source lies on y = constant edge
        if (xflag)
            % Source lies on x= xmin // x = xmax
            SrcData(s).node = find((mesh.nodes(:,2)<=ys+SrcData(s).Radius)&(mesh.nodes(:,2)>=ys-SrcData(s).Radius) ...
                &(mesh.nodes(:,1)==xs)); % Node numbers
        else
            % Source lies on y = ymin// y = ymax
            SrcData(s).node = find((mesh.nodes(:,1)<=xs+SrcData(s).Radius)&(mesh.nodes(:,1)>=xs-SrcData(s).Radius)...
                &(mesh.nodes(:,2)==ys));
        end
    else
        % Internal source
        % considers a circular source with a given radius.
        dcircle=@(p) sqrt((p(:,1)-xs).^2+(p(:,2)-ys).^2)-SrcData(s).Radius;
...
if ~isfield(Det,'Radius')
    Det.Radius = 0.01*min(mesh.hx,mesh.hy); % set default radius to simulate point source
elseif (isempty(Det.Radius))
    Det.Radius = 0.01*min(mesh.hx,mesh.hy);
end
xd = Det.Loc(:,1);yd = Det.Loc(:,2);
fprintf('Ensuring detectors are centred on Nodes... \n')
% push2node = nearestNeighbor(mesh.tri,xd,yd);
% xd = mesh.nodes(push2node,1); yd = mesh.nodes(push2node,2);
...
if Det.ID=='S'
    ...
    for d=1:NofDet
        xflag = (xd(d)==xmin)||(xd(d)==xmax); % '1' - detector lies on x = constant edge ; '0' - detector lies on y = constant edge
        if (xflag)
            % Source lies on x= xmin // x = xmax
            Det.node{d} = find((mesh.nodes(:,2)<=yd(d)+Det.Radius)&(mesh.nodes(:,2)>=yd(d)-Det.Radius)...
                &(mesh.nodes(:,1)==xd(d))); % Node numbers
        else
            % Source lies on y = ymin// y = ymax
            Det.node{d} = find((mesh.nodes(:,1)<=xd(d)+Det.Radius)&(mesh.nodes(:,1)>=xd(d)-Det.Radius)...
                &(mesh.nodes(:,2)==yd(d)));
        end
       
        if isempty(Det.node{d})
            Det.node{d} = nearestNeighbor(mesh.tri,xd(d),yd(d));
        end
    ...
    % considers a circular source with a given radius.
    for d=1:NofDet
        dcircle=@(p) sqrt((p(:,1)-xd(d)).^2+(p(:,2)-yd(d)).^2)-Det.Radius;
...
%         %     if (SrcData(s).radius(s,1) >= mesh.hx)||(SrcData(s).radius(s,2) >= mesh.hy)
...
%         SrcData(s).Loc = [querypoints(:,1),querypoints(:,2)];
%         SrcData(s).elem=pointLocation(mesh.tri,querypoints(:,1),querypoints(:,2));
%         SrcData(s).node=pF;
%         SrcData(s).ID = 'B'; % For beam, i.e. source spans over more than one element
%     else
%         if (SrcData(s).ID =='I')
%             %internal source
...
%             BaryIn(:,1) = (BaryCoords(:,1)>=-eps)&(BaryCoords(:,1)-1<=eps);
%             BaryIn(:,2) = (BaryCoords(:,2)>=-eps)&(BaryCoords(:,2)-1<=eps);
%             BaryIn(:,3) = (BaryCoords(:,3)>=-eps)&(BaryCoords(:,3)-1<=eps);
...
%             BaryCoords = zeros(length(mesh.edgeElem),3);
%
%             BaryCoords(:,1) = ((y2-y3).*(Det.Loc(i,1)-x3) + (x3-x2).*(Det.Loc(i,2)-y3))./det;
%             BaryCoords(:,2) = ((y3-y1).*(Det.Loc(i,1)-x3) + (x1-x3).*(Det.Loc(i,2)-y3))./det;
%             BaryCoords(:,3) = 1-sum(BaryCoords,2);
%             BaryIn(:,1) = (BaryCoords(:,1)>=-eps)&(BaryCoords(:,1)-1<=eps);
%             BaryIn(:,2) = (BaryCoords(:,2)>=-eps)&(BaryCoords(:,2)-1<=eps);
%             BaryIn(:,3) = (BaryCoords(:,3)>=-eps)&(BaryCoords(:,3)-1<=eps);
%             ind = find(sum(BaryIn,2)>=3);
%             Det.elem(i) = mesh.edgeElem(ind(1));
%
%
%         else
%             %internal detectors
...
% end
%
% % remove detectors corresponding to source node:
% for i=1:NofSources
%     remove_det = find(ismember(Det.elem,SrcData(i).elem));
%     if ~isempty(remove_det)
%         if remove_det(1)~=1
%             retain_ind = [1:remove_det-1,remove_det+1:length(Det.elem)];
%         else
%             retain_ind = [remove_det+1:length(Det.elem)];
%         end
%         Det.elem = Det.elem(retain_ind);
%         Det.Loc = Det.Loc(retain_ind,:);
%         Det.nodes = Det.nodes(retain_ind);
%     end
% end
%
