function [Jd,varargout]=FwdSolver_pert(Dimension,N,SimType,mesh,Src,Det,AssemblyFile,pfuncflag,evalFluence,probType,options)
%%
% 23-05-2021 - testing perturbations to forward solver
...
gm_restart = 30;gm_tol = 1e-12; gm_maxit=50;
...
if (SimType)
    ...
    muax=zeros(1,1,Elem);
    %set total muax, muam
    omega_by_c = (1i*Src(1).mfreq/mesh.opt.speed);
    muax(1,1,:)=mesh.opt.muaxi(1,1,:)+mesh.opt.muaxf(1,1,:)+omega_by_c;
    
    switch(pfuncflag)
        case 0
            bn = get_pfunc_coeff(pfuncflag,deflag,mesh.opt.gx);
        case 1
            bn = get_pfunc_coeff(pfuncflag,deflag,mesh.opt.gx,mesh.opt.gx, mesh.opt.gamma);
        case 2
            bn = get_pfunc_coeff(pfuncflag,deflag,mesh.opt.gx,mesh.opt.gx,mesh.opt.alpha,mesh.opt.gbx);
    ...
    muam(1,1,:)=mesh.opt.muaimult*mesh.opt.muaxi(1,1,:)+mesh.opt.muafmult*mesh.opt.muaxf(1,1,:) + omega_by_c;
    
    switch(pfuncflag)
        case 0
            bn = get_pfunc_coeff(pfuncflag,deflag,mesh.opt.gm);
        case 1
            bn = get_pfunc_coeff(pfuncflag,deflag,mesh.opt.gm,mesh.opt.gm,mesh.opt.gamma);
        case 2
            bn = get_pfunc_coeff(pfuncflag,deflag,mesh.opt.gm, mesh.opt.gm,mesh.opt.alpha, mesh.opt.gbm);
    end
    ...
    muax=zeros(1,1,Elem);
    omega_by_c = (1i*Src(1).mfreq/mesh.opt.speed);
    muax(1,1,:)=mesh.opt.muaxi(1,1,:)+omega_by_c;
    
    switch(pfuncflag)
        case 0
            bn = get_pfunc_coeff(pfuncflag,deflag,mesh.opt.gx);
        case 1
            bn = get_pfunc_coeff(pfuncflag,deflag,mesh.opt.bngx,mesh.opt.gx,mesh.opt.gamma);
        case 2
            bn = get_pfunc_coeff(pfuncflag,deflag,mesh.opt.bngx,mesh.opt.gx,mesh.opt.alpha, mesh.opt.hx);
    end
    ...
    if (isa(AssemblyFile,'function_handle'))
        disp('Obtain FEM matrices...')
        [Ks,Km,Kb,Iv,Jv,elem_area] = AssemblyFile(mesh);
    else
        load(AssemblyFile,'Ks','Km','Kb','Iv','Jv');
    end
...
[status, msg, msgID] = mkdir('storedlibrary');
save('storedlibrary/backgroundFEM.mat',...
    'Kb','Ks','Km','PhiX','AX','Iv','Jv',...
    'Dimension','N','CgradphiX','CphiX','CgradbphiX','CbphiX',...
    'N','Elem','mesh','muax','bn','Src','Det','Dimension',...
    'SX','Jd','CJX','omega_by_c','gm_restart', 'gm_tol', 'gm_maxit');
