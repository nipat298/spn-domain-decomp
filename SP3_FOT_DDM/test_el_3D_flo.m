...
h=[0.05,0.05,0.05]; %Mesh spacing for structured mesh. Recommended to use same mesh size in each dimension, 
                ...
                % [hx,hy] or [hx,hy,hz]
Domain=[-1,1,-1,1,0,2];
% Domain=[-1.5,1.5,-1.5,1.5,0,3];  %[xmin,xmax,ymin,ymax] or [xmin,xmax,ymin,ymax,zmin,zmax] respectively
...
Background_property=[0.1,100,0.9,1.37,0.9,0.9,1.33];%[mua_i, mus,muai_mult, mus_mult,g_x,g_m,refr_ind]; % One row for each layer
Fluorophore_property=[0.006,1,1,0.016,1e-9];
