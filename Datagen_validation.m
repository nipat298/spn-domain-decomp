function [mesh,Jd,Fluence,varargout]=Datagen_validation(simulation,SettingsFile,func,fcs_flag,varargin)
...
% Fluence - Array of size 2*NofNodes X 1. [FX;FM] - Concatenated array
...
% This part works only for structured meshes. The Assembly codes will have
% to be correspondingly modified for unstructured/adaptive meshes
if Dimension==3
...
if simulation =='F'
    ...
    % These parameters are not relevant when using the Henyey Greenstein
    % function. 
...
if fcs_flag==1
    ...
    for s=1:NofSources
        if (Src(s).AOI)~=90
            % For collimated incidence, we move the source 1 mean free path
            % inside the medium. The source is now marked as an internal
            % source
            fprintf('Moving source %d one mean free path inside the medium \n',s);
            temp =180-Src(s).AOI;
            if Src(s).Loc(1,1)==Domain(1)
...
if ~fcs_flag
    ...
    % fcs/ de variants
    [phi_u,Jd,Fluence]=func(Dimension,N,SimType,mesh,Src,Det,AssemblyFile,pfuncflag,evalFluence,probType);
    varargout{1}=phi_u;
end
