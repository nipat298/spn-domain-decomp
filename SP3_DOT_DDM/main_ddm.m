...
%mesh.opt is sub-structure that contains the optical properties of the medium
if simulation =='F'
...
for s=1:NofSources
    if strcmp(Src(s).Type,'collimated')
        ...
        elseif Src(s).Loc(1,2)==Domain(4)
            ...
            Src(s) = get_one_mfp_source(mesh,Src(s),mfp);
    end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% Domain decomposition integration begins from here %%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% lambda=0.32;
% beta=1;

lambda=(1/(0.25*mesh_spacing));
beta=1/(2*(mesh_spacing^2));

NoDomains=4; % Defines number of domains; all the subdomains are of same size
int_axes='x'; % Defines axis along which subdomains need to be formed

% Here, partmat is a structure type variable.
partmat= ddm_interfacing(mesh,NoDomains,int_axes,Domain);

[Jd_pert,PhiX_pert]=FwdSolver_DDM(Dimension,N,SimType,mesh2,partmat,Src,Det,lambda,beta,AssemblyFile,solver_options);
