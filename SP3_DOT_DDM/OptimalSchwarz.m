function partmat=OptimalSchwarz(partmat,lambda,N)

% This function implements the domain decomposition algorithm
...
% For better understanding of this subroutine, refer the text file "Detailed code layout" shared with the folder
tic
tol = 1e-6;
maxit = 1000;
...
while max_tol > 0.01 || iter<=2 
    ...
    parfor i=1:NoDomains
        ...
        RHS_temp=zeros(((N+1)/2)*len_node,1); % size 2N for SP3 case 
        for j=1:NoDomains
            if j==i
                continue
            else
               ...
               RHS_temp=RHS_temp+(partmat(i).RHS_extra{j}*partmat(i).robin{j}); % This calculation is done for each interface of a subdomain
        ...
        partmat(i).solnorm=norm(partmat(i).soln-partmat(i).uprev);
    end
   ...
   % interfaces of a given subdomain
    u_int=cell(NoDomains,NoDomains); % Just like g_ini, we initialize a cell structure for u_int
    for i=1:NoDomains
        len_node=length(partmat(i).nodes);
        for j=1:NoDomains
            if j==i
