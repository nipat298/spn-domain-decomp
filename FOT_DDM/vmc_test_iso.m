...
Background_property=[0.05,50,1,1,0.9,0.9,1.33];%[mua_i, mus,muai_mult, mus_mult,g_x,g_m,refr_ind]; % One row for each layer
Fluorophore_property=[0,1,1,0.016,1e-9]; 
%Fluorophore_property=[0.006,1,1,0.016,1e-9]; %[mua_f, muaf_mult, ContrastRatio, Quant_eff, Fluor_lifetime]; %One row for each fluorophore 
Inhom_property=[0.147]; % Specify one row for each inhomogeneity
...
Source(1).mfreq = 0; % Modulation frequency
...
Source(1).Radius = 0.0250; % set beam radius. For pencil beam set radius less than min(hx,hy)
%Source(1).Radius = 0.00200;
% Source(1).beam = [];
%% Detector properties
