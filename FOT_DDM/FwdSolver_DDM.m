function [Jd,varargout]=FwdSolver_DDM(Dimension,N,SimType,mesh,partmat,Src,Det,alpha,beta,AssemblyFile,options)
%%
...
%     end
    ...
    for ii=1:NoDomains
     ...
     disp('Generate Sparse Assembly matrix for emission... ')
        ...
        partmat(ii).Gsub=partmat(ii).Gsub+partmat(ii).LHS;
         %partmat(ii).SM=GetAssembledMat_ddm(sub_nodes,Dimension,N,Iv,Jv,Nodes,Mbeta(:,:,elem_sub),Km);
        partmat(ii).SM=GetAssembledMat_ddm(sub_nodes,Dimension,N,Iv,Jv,Nodes,Mbeta(:,:,elem_sub),Km)*partmat(ii).Phi_emission;
        
    end
    
    partmat=OptimalSchwarz_FOT(partmat,alpha,N);
